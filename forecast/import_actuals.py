...
# Sample chart of account entre
# '3000-30000-109189-52191003-310940-00000-00000-0000-0000-0000'
# The following are the index for the chart of account after decoded into a list
DIT_INDEX = 0  # must be 3000. If not, the trial balance is for another department
CC_INDEX = 2
NAC_INDEX = 3
PROGRAMME_INDEX = 4
ANALYSIS1_INDEX = 5
...
def check_trial_balance_format(file, period):
    """Check that the file is really the trial balance and it is the correct period"""
    pass


def row_to_use(row):
    """Check the account code: if resource or capital use the row, otherwise ignore it"""
...
def save_row(chart_of_account, value, period_obj, year_obj):
    """Parse the long strings containing the chart of account information. Return errors if missing from database."""
    # '3000-30000-109189-52191003-310940-00000-00000-0000-0000-0000'
    chart_account_list = chart_of_account[CHART_OF_ACCOUNT_COL].split(CHART_ACCOUNT_SEPARATOR)
    error_message = ''
    ...
    if error_message == '':
        adiobj, created = MonthlyFigure.objects.get_or_create(financial_year=year_obj,
...
def upload_file(path, period, year):
    """Use transactions, so it can rollback if there is an error in the file"""
    ...
    # If we are here, the file is a trial balance run for the correct period

...
class SegmentParent(TimeStampedModel):
    ...
    def __str__(self):
        return "{} - {}".format(
            self.segment_parent_code,
            self.segment_parent_long_name,
        )
...
class Segment(TimeStampedModel):
    segment_code = models.CharField(
        max_length=8,
        primary_key=True,
        verbose_name='segment code',
    )
    segment_long_name = models.CharField(
        max_length=255,
        verbose_name='segment long name',
    )
    segment_parent_code = models.ForeignKey(
        SegmentParent,
        on_delete=models.PROTECT,
    )
...
class SubSegment(TimeStampedModel):
    ...
    sub_segment_long_name = models.CharField(
        max_length=255,
        verbose_name='sub segment long name',
    )
    Segment_code = models.ForeignKey(
        Segment,
        on_delete=models.PROTECT,
    )
    ...
    estimates_row_code = models.ForeignKey(
        EstimateRow,
        on_delete=models.PROTECT,
    )
    ...
    def __str__(self):
        return "{} - {}".format(
            self.sub_segment_code,
            self.sub_segment_long_name,
        )
